...
public class PyObjectAnnotator : IAnnotator
{
    public void Annotate(AnnotationCollection annotations)
    {
        ...
        using (Py.GIL())
        {
            using PyObject mod = Py.Import(pythonTypeAttr.Module);
            using var typeAttr = mod.GetAttr(pythonTypeAttr.TypeName);
            using var type = new PyType(typeAttr);
            if (type.HasAttr("_member_map_"))
            {
                // Handling enum: enums has _member_map_.
                using var member_map = type.GetAttr("_member_map_");
                try
                {
                    // in some pythons this is not a dict but a OrderedDict.
                    // so we have to manually pull the values
                    var list = new List<PyObject>();
                    using var valuesObj = member_map.InvokeMethod("values");
                    using var values = new PyIterable(valuesObj);
                    foreach (var pair in values)
                    {
                        using var nameAttr = pair.GetAttr("name");
                        list.Add(type.GetAttr(nameAttr));
                        pair.Dispose();
                    }
